...
class PartnerCenterAPIClient:
    ...
    def filter_invoices(self, invoice_ids: list[dict]) -> list[dict]:
        """
    ...
    def write_to_blob_storage(self, df:BytesIO):
        """
...
def main():
    ...
    invoice_ids:list[str] = api_client.get_invoice_ids()
    # print(invoice_ids)
    ...
    print(matching_invoice_ids)
  

    final_df=pd.DataFrame()

    for idx,invoices in enumerate(matching_invoice_ids):
        # if idx<=3:
        invoice_id = invoices
        line_items = api_client.get_invoice_line_items(invoice_id)
        temp_df=pd.DataFrame(line_items)
        # temp_df["billing_month"] = temp_df["chargeStartDate"]
        print(f"Made the {idx} df")
        final_df=pd.concat([final_df,temp_df])
        # else:
        #     break

    print("making the bytes df")
    
    final_df = bytes(final_df.to_csv(index=False), encoding='utf-8')
    print(type(final_df))
